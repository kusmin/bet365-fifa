...
while True:
    ...
    for league in Ligas:
        ...
        while pagina <= 1000:
            ...
            URL = f'https://api.b365api.com/v3/events/ended?sport_id=1&page={pagina}&day=TODAY&league_id={league}&token={TOKEN}'
            ...
            try:
                for dado in dados:
                    ...
                    resultado = lista['ss'].replace("'",'')
                    data = converterData(lista['time'].replace("'",''))

                    if SelectBD().selectJogo(idJogo,1) == None:
                        InsertBD().insertDados(idJogo, jogadorA, jogadorB, timeA, timeB, resultado, data, liga(league))
                        print('==============')
                        print('Dado Gravado')
            except:
                print('-------')
