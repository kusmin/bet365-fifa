...
class SelectBD():
    ...
    def selectNJogos(self, jogadorA: str, jogadorB: str):
        jogadorA = jogadorA.replace('ECF_','')
        jogadorB = jogadorB.replace('ECF_', '')
        ...
        # Exucuta a instrução sql
        cursor.execute(
            f"SELECT * FROM bd_atos.dados WHERE jogadorA='{jogadorA}' and jogadorB='{jogadorB}' OR jogadorA='{jogadorB}' and jogadorB='{jogadorA}' ORDER BY dados.data DESC LIMIT 10 ;")
