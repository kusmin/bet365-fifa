...
ws.cell(1, 9, 'Média ultimos 10 dias')
ws.cell(1, 10, 'Média dos últimos 10 jogos')
ws.cell(1, 11, 'Média Geral com esses times')
ws.cell(1, 12, 'Qtd de Jogos')
ws.cell(1, 13, '% Ambos Marcam Geral')
ws.cell(1, 14, '% Ambos Marcam com Estes Times')
ws.cell(1, 15, '% Vitoria Casa')
ws.cell(1, 16, '% Vitoria Visitante')
ws.cell(1, 17, '% Empate')
...
for linha in dadosFuturos:
    ...
    ws.cell(Linha, 6, str(linha[6])[0:10])
    ws.cell(Linha, 7, str(linha[6])[11:16])
    ws.cell(Linha, 8, getMediaGeral(linha[1], linha[2]))
    #ws.cell(Linha, 9, getMediaSeteDias(linha[1], linha[2]))
    ws.cell(Linha, 9, getMediaDezDias(linha[1], linha[2]))
    ws.cell(Linha, 10, getMediaNJogos(linha[1], linha[2]))
    ...
    ws.cell(Linha,12, len(dadosVerify))
    ws.cell(Linha,13, f'{porcAmbasMarcam(linha[1],linha[2]) * 100}%')
    ws.cell(Linha, 14, f'{getMediaAmbasMarcam(dadosVerify) * 100}%')
    ws.cell(Linha, 15, f'{getPorcCasa(linha[1],linha[2]) * 100}%')
    ws.cell(Linha, 16, f'{getPorcVisitante(linha[1],linha[2]) * 100}%')
    ws.cell(Linha, 17, f'{getPorcEmpate(linha[1],linha[2]) * 100}%')
